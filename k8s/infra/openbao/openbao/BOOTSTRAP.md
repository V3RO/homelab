...
**`openbao-admin`'s own password needs no Bitwarden item at all** — it's
generated by OpenBao itself (`RandomSecret`, same file), then relayed into
the `cnpg-openbao-admin-credentials` Secret that CNPG's `managed.roles`
reads via the shared `openbao` `ClusterSecretStore` (see
[external-secrets/openbao-config](../../external-secrets/openbao-config)) —
that store and its bootstrap policy/role exist once, ever, and cover every
future app's OpenBao-generated credential the same way, not just this one.
...
**Is this needed for every database?** For any *future* CNPG cluster built
from a template that includes these bootstrap hooks, no — CNPG's own
internal bootstrap superuser (not exposed to us, but usable during initial
cluster creation) runs this automatically. It's only required, once, for
each cluster that already existed *before* this pattern was adopted —
`authentik-postgres` now, and the same one-time command again for each of
grafana/immich/n8n/mealie/forgejo/etc. when they're migrated. That's a
fixed, bounded cost (one command per pre-existing app, ever) rather than a
recurring one. It could theoretically be automated by temporarily
flipping `enableSuperuserAccess` on each cluster and running the grant via
a Job — not worth doing: that's a bigger, more privileged lever than the
one-time command it would save.

Once the grant is applied, `DatabaseSecretEngineConfig`/`DatabaseSecretEngineStaticRole`
should reconcile successfully — check with `kubectl -n authentik get
databasesecretengineconfig,databasesecretenginestaticrole,randomsecret`.
